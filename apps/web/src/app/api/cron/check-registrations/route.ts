...
// notifikaciju (vozila I klijenti), I periodični izvještaj o floti -
...
export async function GET(request: Request) {
  ...
  const [registrationExpiry, incompleteVehicleData, incompleteClientData, periodicReport] = await Promise.all([
    runRegistrationExpiryCheck(),
    runIncompleteVehicleDataCheck(),
    runIncompleteClientDataCheck(),
    runPeriodicReportCheck(),
  ...
  return NextResponse.json({ registrationExpiry, incompleteVehicleData, incompleteClientData, periodicReport });
}
